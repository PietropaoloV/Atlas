  ...
  async handle(req) {
    try {
     ...
     await req.on('end', () => {});
      switch (req.method) {
        case "POST":
          return this.CreateWorkout(req.url.split('/')[1]);
        case "GET":
          if (req.url.split('/').length == 4) {
            return this.GetWorkout(req.url.split('/')[1], req.url.split('/')[3]);
          } else {
            return this.ListWorkout(req.url.split('/')[1]);
          }
        case "PUT":
          return this.UpdateWorkout(req.url.split('/')[1], req.url.split('/')[3]);
        case "DELETE":
          return this.DeleteWorkout(req.url.split('/')[1], req.url.split('/')[3]);
        default:
