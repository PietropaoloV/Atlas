...
const { getClientToken, getServerToken } = require('../constants/server-constants');
...
class ExerciseHandler extends Handler {
	...
	async handle(req) {
        try {
			if (getServerToken() !== getClientToken()) {
        	...
        	await req.on('end', () => {});
          	switch (req.method) {
				case "POST":	
					return this.CreateExercise(req.url.split('/')[1]);
				case "GET":
				if (req.url.split('/').length == 4) {
					return this.GetExercise(req.url.split('/')[1], req.url.split('/')[3]);
				} else {
					var resp =  this.ListExercise(req.url.split('/')[1]);
					console.log(resp);
					return resp;
				}
				case "PUT":
				return this.UpdateExercise(req.url.split('/')[1], req.url.split('/')[3]);
				case "DELETE":
				return this.DeleteExercise(req.url.split('/')[1], req.url.split('/')[3]);
				default:
