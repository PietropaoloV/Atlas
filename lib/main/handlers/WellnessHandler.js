...
class WellnessHandler extends Handler {
  async handle(req) {
    try {
      ...
      switch (req.method) {
        case "POST":
          return this.CreateWellness(req.url.split('/')[1]);
        case "GET":
          if (req.url.split('/').filter(Boolean).length == 3) {
            return this.GetWellness(req.url.split('/')[1], req.url.split('/')[3]);
          } else {
            return this.ListWellness(req.url.split('/')[1]);
          }
        case "PUT":
          return this.UpdateWellness(req.url.split('/')[1], req.url.split('/')[3]);
        case "DELETE":
          return this.DeleteWellness(req.url.split('/')[1], req.url.split('/')[3]);
        default:
